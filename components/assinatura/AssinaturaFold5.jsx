...
export default function AssinaturaFold6() {
  ...
  return (
    <section className="bg-[#F8F8FE] py-[60px] flex justify-center w-full" id="investimento">
      <div className="max-w-[1200px] w-full px-[45px] flex flex-col items-center">
        ...
        <div className="flex flex-col items-center text-center gap-5 mb-[60px] max-w-[570px]">
          <div>
            ...
            <h2 className="text-[40px] font-bold text-[#1a1a1a] leading-[46px] tracking-[-0.03em] mb-4">
              Comece agora mesmo
            </h2>
            <p className="text-[#1a1a1a] opacity-70 text-[18px] font-semibold leading-[150%]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-[23.76px] w-full max-w-[1110px] items-start">
          {plans.map((plan, index) => (
            <div key={index}>
              <div 
                className={`flex flex-col justify-between p-5 h-[454px] rounded-[20px] transition-all relative
                  ${plan.highlight 
                ...
                <div className="flex flex-col gap-[13.73px]">
                  <div className="flex justify-between items-start w-full">
                    <span className={`px-[13.7px] py-[6.8px] rounded-[31.5px] text-[20px] font-medium tracking-tight
                      ${plan.highlight ? 'bg-[#004DFF] text-white' : 'bg-[#F8F8FE] text-[#262626]'}`}>
                      {plan.name}
                    </span>
                    <span className="text-[13.7px] font-bold text-[#1a1a1a] text-right max-w-[137px] leading-none">
                      {plan.validity}
                    </span>
                  </div>
                  ...
                  <div className="flex items-end gap-1.5 h-[52px]">
                    {plan.price === "Sob consulta" ? (
                      <span className="text-[42px] font-semibold tracking-tight text-[#1a1a1a]">
                        {plan.price}
                      </span>
                    ) : (
                      <>
                ...
                <div className="flex flex-col gap-1.5 mt-6 mb-8">
                  {plan.features.map((feature, fIndex) => (
                    <div key={fIndex} className="flex items-start gap-3">
                      ...
                      <span className="text-[17.2px] font-semibold text-[#1a1a1a] leading-[20px] tracking-tight">
                        {feature}
                      </span>
                    </div>
                  ))}
